...
export default function InterviewPage() {
  ...
  // init first question if none.
  useEffect(() => {
    if (!currentQuestion) {
      setCurrentQuestion("Tell me about a typical day in your role. What are the first things you check?", "anchor");
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-cream flex">
      {/* Sidebar */}
      <aside className="w-[220px] bg-white border-r border-cream-dark p-6 hidden md:block">
        <div className="mb-8">
          ...
          <p className="text-xs text-text-light uppercase tracking-tighter">{user?.job_title}</p>
        </div>

        <nav className="space-y-4">
          {[1, 2, 3, 4, 5, 6].map(num => (
            <div key={num} className="flex items-center space-x-3 opacity-60">
      ...
      <div className="flex-grow flex flex-col">
        <header className="p-6 flex justify-between items-center border-b border-cream-dark">
          ...
          <div className="label-caps bg-amber-light px-3 py-1 rounded text-amber">Session 3: Processes</div>
        </header>
        ...
        <main className="flex-grow p-12 max-w-3xl mx-auto w-full">
          ...
          <div className="space-y-6">
            <textarea
              ...
              className="w-full h-48 p-6 bg-white border border-cream-dark rounded-lg focus:ring-1 focus:ring-green-mid outline-none resize-none text-lg shadow-inner"
              disabled={isStreaming}
            />
            
            <div className="flex justify-between items-center">
              <button className="text-text-light hover:text-text-dark transition-colors">Skip for now</button>
              <button 
